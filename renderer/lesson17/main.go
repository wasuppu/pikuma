...
func update() {
	...
	for i := range len(mesh.faces) {
		...
		for j := range 3 {
			...
			// Use a matrix to scale, rotate and translate our original vertex, the order matters
			transformedVertex = scaleMatrix.mulv(transformedVertex)
			transformedVertex = rotationMatrixX.mulv(transformedVertex)
			transformedVertex = rotationMatrixY.mulv(transformedVertex)
			transformedVertex = rotationMatrixZ.mulv(transformedVertex)
			transformedVertex = translationMatrix.mulv(transformedVertex)
		...
		for j := range 3 {
			...
			projectedPoints[j] = transformedVertices[j].v3().project()
		...
		projectedTriangle := Triangle{projectedPoints, meshFace.color, avgDepth}
