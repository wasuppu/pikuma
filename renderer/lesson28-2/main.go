...
func setup() (err error) {
	...
	// Loads mesh entities
	err = LoadMesh(filepath.Join(parentpath, "assets", "runway.obj"), filepath.Join(parentpath, "assets", "runway.png"), Vec3{1, 1, 1}, Vec3{0, -1.5, 23}, Vec3{0, 0, 0})
	if err != nil {
		return fmt.Errorf("failed to load model: %s", err)
	}
	err = LoadMesh(filepath.Join(parentpath, "assets", "f22.obj"), filepath.Join(parentpath, "assets", "f22.png"), Vec3{1, 1, 1}, Vec3{0, -1.3, 5}, Vec3{0, -math.Pi / 2, 0})
	if err != nil {
		return fmt.Errorf("failed to load model: %s", err)
	}
	err = LoadMesh(filepath.Join(parentpath, "assets", "efa.obj"), filepath.Join(parentpath, "assets", "efa.png"), Vec3{1, 1, 1}, Vec3{-2, -1.3, 9}, Vec3{0, -math.Pi / 2, 0})
	if err != nil {
		return fmt.Errorf("failed to load model: %s", err)
	}
	err = LoadMesh(filepath.Join(parentpath, "assets", "f117.obj"), filepath.Join(parentpath, "assets", "f117.png"), Vec3{1, 1, 1}, Vec3{2, -1.3, 9}, Vec3{0, -math.Pi / 2, 0})
	if err != nil {
		return fmt.Errorf("failed to load model: %s", err)
	}
...
func processGraphicsPipelineStages(mesh Mesh) {
	...
	for i := range len(mesh.faces) {
		...
		faceNormal := getTriangleNormal(transformedVertices)

		// Backface culling test to see if the current face should be projected
		if cullMethod == CULL_BACKFACE {
			// Find the vector between vertex A in the triangle and the camera origin
			origin := Vec3{0, 0, 0}
			cameraRay := origin.sub(transformedVertices[0].v3())

			// Calculate how aligned the camera ray is with the face normal (using dot product)
			dotNormalCamera := faceNormal.dot(cameraRay)
		...
		for t := range numTrianglesAfterClipping {
			...
			lightIntensityFactor := -faceNormal.dot(light.direction)
