...
func drawTrianglePixel(x, y int32, color uint32, pointA, pointB, pointC Vec4) {
	...
	if interpolatedReciprocalW < getZBuffer(x, y) {
...
func drawTexel(
	...
	textureWidth := img.textureWidth
	textureHeight := img.textureHeight
	...
	if interpolatedReciprocalW < getZBuffer(x, y) {
		// Draw a pixel at position (x,y) with the color that comes from the mapped texture
		drawPixel(x, y, img.texture[textureWidth*texY+texX])
...
func drawTexturedTriangle(
	...
	if y1-y0 != 0 {
		for y := y0; y <= y1; y++ {
			...
			for x := xStart; x < xEnd; x++ {
				// Draw our pixel with the color that comes from the texture
				drawTexel(x, y, img, pointA, pointB, pointC, auv, buv, cuv)
			}
		}
	}
...
func clearColorBuffer(color uint32) {
	for i := range windowWidth * windowHeight {
		colorBuffer[i] = color
	}
}
...
func getZBuffer(x, y int32) float64 {
	if x < 0 || x >= windowHeight || y < 0 || y >= windowHeight {
		return 1.0
	}
	return zbuffer[windowWidth*y+x]
}

func updateZBuffer(x, y int32, value float64) {
	if x < 0 || x >= windowHeight || y < 0 || y >= windowHeight {
		return
	}
