...
type Img struct {
	texture       []uint32 // mesh PNG texture pointer
	textureWidth  int
	textureHeight int
}
...
type Mesh struct {
	...
	img         Img    // mesh PNG texture pointer
...
func LoadMesh(objFilename, pngFilename string, scale, translation, rotation Vec3) error {
	mesh, err := LoadObjFileData(objFilename)
	if err != nil {
		return fmt.Errorf("failed to load model: %s", err)
	}
	meshTexture, textureWidth, textureHeight, err := LoadPngTextureData(pngFilename)
	if err != nil {
		return fmt.Errorf("failed to load texture from png: %s", err)
	}

	mesh.img = Img{meshTexture, textureWidth, textureHeight}
	mesh.scale = scale
