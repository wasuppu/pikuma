...
func drawTexel(
	...
	pointA, pointB, pointC Vec2,
	u0, v0, u1, v1, u2, v2 float64,
) {
	...
	texX := int(math.Abs(interpolatedU * float64(textureWidth)))
	texY := int(math.Abs(interpolatedV * float64(textureHeight)))

	texX = clamp(texX, 0, textureWidth-1)
	texY = clamp(texY, 0, textureHeight-1)
...
func drawTexturedTriangle(
	x0, y0 int32, u0, v0 float64,
	x1, y1 int32, u1, v1 float64,
	x2, y2 int32, u2, v2 float64,
	texture []uint32,
) {
	// We need to sort the vertices by y-coordinate ascending (y0 < y1 < y2)
	if y0 > y1 {
