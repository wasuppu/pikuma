...
func (app *Application) Setup() {
	...
	bigBall := physics.NewBody(physics.NewCircleShape(200), float64(app.graphic.windowWidth)/2, float64(app.graphic.windowHeight)/2, 0.0)
	app.bodies = append(app.bodies, bigBall)
}
	...
	for event := sdl.PollEvent(); event != nil; event = sdl.PollEvent() {
		...
		case *sdl.MouseButtonEvent:
			if t.Type == sdl.MOUSEBUTTONDOWN {
				...
				smallBall := physics.NewBody(physics.NewCircleShape(40), float64(x), float64(y), 1.0)
				smallBall.Restitution = 0.2
				app.bodies = append(app.bodies, smallBall)
			}
		}
	}
}
...
func (app *Application) Render() {
	...
	for _, body := range app.bodies {
		...
		case physics.CIRCLE_SHAPE:
			circleShape := body.Shape.(*physics.CircleShape)
			app.graphic.DrawFillCircle(int32(body.Position.X), int32(body.Position.Y), int32(circleShape.Radius), 0xFFFFFFFF)
		case physics.BOX_SHAPE:
			boxShape := body.Shape.(*physics.BoxShape)
			app.graphic.DrawPolygon(int32(body.Position.X), int32(body.Position.Y), boxShape.WorldVertices, 0xFFFFFFFF)
		}
	}
