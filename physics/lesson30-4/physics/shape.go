...
func NewPolygoShape(vertices []Vec2) *PolygonBase {
	...
	polygon.width = maxX - minX
	polygon.height = maxY - minY
...
func (s *PolygonBase) GetMomentOfInertia() float64 {
	...
	for i := range s.LocalVertices {
		a := s.LocalVertices[i]
		b := s.LocalVertices[(i+1)%len(s.LocalVertices)]
...
func (s *PolygonBase) Width() float64 {
	return s.width
}

func (s *PolygonBase) Height() float64 {
	return s.height
}
...
func NewBoxShape(width, height float64) *BoxShape {
	box := BoxShape{PolygonBase: PolygonBase{width: width, height: height}}
