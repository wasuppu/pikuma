	...
	for i := range len(app.bodies) {
		for j := i + 1; j < len(app.bodies); j++ {
			...
			if isColliding, contact := physics.IsColliding(a, b); isColliding {
				...
				minCurrVertex, minNextVertex := contact.Start, contact.End
				app.graphic.DrawFillCircle(int32(minCurrVertex.X), int32(minCurrVertex.Y), 5, 0xFF00FFFF)
				app.graphic.DrawFillCircle(int32(minNextVertex.X), int32(minNextVertex.Y), 5, 0xFF00FFFF)
			}
		}
	}
}
