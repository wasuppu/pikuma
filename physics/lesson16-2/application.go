...
func (app *Application) Setup() {
	...
	box := physics.NewBody(physics.NewBoxShape(200, 100), float64(app.graphic.windowWidth)/2, float64(app.graphic.windowHeight)/2, 1)
...
func (app *Application) Render() {
	...
	for _, body := range app.bodies {
		...
		case physics.CIRCLE_SHAPE:
			circleShape := body.Shape.(*physics.CircleShape)
			app.graphic.DrawCircle(int32(body.Position.X), int32(body.Position.Y), int32(circleShape.Radius), body.Rotation, 0xFFFFFFFF)
