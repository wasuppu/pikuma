...
func NewBody(shape Shape, x, y, mass float64) *Body {
	...
	restitution := 1.0
	friction := 0.7
	return &Body{Position: Vec2{x, y}, Mass: mass, InvMass: invMass, Shape: shape, I: I, InvI: invI, Restitution: restitution, Friction: friction}
}
...
func (b *Body) ApplyImpulse(j Vec2) {
	if b.IsStatic() {
		return
	}
...
func (b *Body) ApplyImpulse2(j, r Vec2) {
	if b.IsStatic() {
		return
	}
...
func (b *Body) Update(dt float64) {
	b.IntegrateLinear(dt)
	b.IntegrateAngular(dt)
	isPolygon := b.Shape.GetType() == POLYGON_SHAPE || b.Shape.GetType() == BOX_SHAPE
	if isPolygon {
		b.Shape.(PolygonShape).UpdateVertices(b.Rotation, b.Position)
	}
}

func (b *Body) IntegrateLinear(dt float64) {
	if b.IsStatic() {
		return
	}
...
func (b *Body) IntegrateAngular(dt float64) {
	if b.IsStatic() {
		return
	}
